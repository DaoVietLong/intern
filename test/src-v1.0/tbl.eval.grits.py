...
def main(args):
    ...
    true_dict = eval_tool.collect_html(args.gt_dir, is_gt = False)
    ...
    for docid in true_dict.keys():
        ...
        true_html = true_dict[docid]
        print ("pred_html=", pred_html)
        print ("true_html=", true_html)
        
        score = grits.grits_from_html(true_html, pred_html)
        print ("score=", score)
        docids.append(docid)
        scores.append(score)
        break
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--output_file',
        type=str,
        default='grits_scores-wo-text-small.npy',
        help=''
    )
